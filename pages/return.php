...
<head>
	...
	<title>Fresh :: Add Return Product</title>
...
<body>
		...
		<div class="row">
				...
				<form action="" class="form-horizontal" id="frmBrand">
					<form action="" class="form-horizontal" id="frmBrand">
						...
						<div class="form-group" align="right" id="frmVendor">
							<label class="col-md-3">Return From</label>
					...
					<table class="table table-bordered">
						...
						<tr>
							<td>
								...
								<select class="form-control" type="text" name="procode" id="procode" placeholder="procode" required>
									<option value="">--Select Product Here--</option>
									<?php
										$dd = $conn->query("SELECT * FROM product");
										while($ddd = $dd->fetch_assoc()):
									?>
									<option value="<?php echo $ddd['barcode']?>"><?php echo $ddd['product_name']?></option>
									<?php endwhile;?>
								</select>
							</td>
							<td>
			...
			<div class="col-md-2">
				<div class="panel-body">
					...
					<div class="form-group" align="left">
						<label>Pay</label>
						<input class="form-control" type="text" name="pay" id="pay" placeholder="pay" disabled>
					</div>
					<div class="form-group" align="left">
						<label>Due</label>
	...
	<script>
		...
		function getVendor(){
			$.ajax({
				type: 'GET',
				url: '../php/vendor/get_lifting_value.php',
				dataType: 'JSON',
		...
		function addROw(product){
			...
			if($('#procode').val().length == 0){
				...
				$.alert({
					title: 'Error',
					content: 'Please Choose the Return From',
					type: 'red',
					autoClose: 'ok|2000'
				});
			}
		...
		function AddInvoice(){
			...
			$.ajax({
				method: 'POST',
				url: '../php/product/add_return.php',
				dataType: 'JSON',
				data: { vendor: $('#vendor').val(), total: $('#total').val(), pay: $('#pay').val(), due: $('#due').val() ,  data: table_data   },
				success: function(data){
					$.alert({
					title: 'Success',
					content: 'Successfully Return Product added',
					type: 'green',
					autoClose: 'ok|2000'
				});
				}
